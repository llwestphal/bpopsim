...
int main(int argc, char* argv[])
{
  ...
  ("initial-population-size,i", "Initial Population Size", uint32_t(5E6))
    ...
    for (uint32_t on_run=0; on_run < from_string<uint32_t>(options["replicates"]); on_run++)
    {
      ...
      if (g_ro_only) {
        ...
        if( print_screen ) {
          ...
          population.PrintFrequenciesToScreen(options["output-folder"], &frequencies);
        }
        
        if( print_max_diff ) {
          std::cout << std::endl << std::endl << "Printing max difference of relevant mutations.... " << std::endl;
          population.CalculateSimilarity(options["output-folder"], frequencies);
        }
    
        if( print_time_to_sweep ) {
          std::cout << std::endl << std::endl << "Printing time to sweep.... " << std::endl;
          population.TimeToSweep(options["output-folder"], frequencies);
        }
        
        if( on_run == 0 ) {
          if( print_freq ) {
            std::cout << std::endl << std::endl << "Printing to file.... " << std::endl;
            population.PrintOut(options["output-folder"], &frequencies);
          }
          ...
          if( print_muller ) {
            ...
            population.DrawMullerMatrix(options["output-folder"], muller_matrix, frequencies);
          }
        }
      }
    }
